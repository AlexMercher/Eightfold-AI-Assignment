...
class LinkedinComparator(BaseComparator):
    def matches(self, candidate_a: Candidate, candidate_b: Candidate) -> ComparisonResult:
        urls_a = {p.url for p in candidate_a.social_profiles if p.platform.lower() == "linkedin"}
        urls_b = {p.url for p in candidate_b.social_profiles if p.platform.lower() == "linkedin"}
