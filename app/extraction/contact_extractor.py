...
from app.pipeline.normalization.normalizers.url import select_canonical_github_profile
...
class ContactExtractor:
    ...
    def extract(
        ...
        Args:
            ...
            github_url:    Optional verified github URL from hyperlink annotations
            linkedin_url:  Optional verified linkedin URL from hyperlink annotations
            portfolio_url: Optional verified portfolio URL from hyperlink annotations
        ...
        # ── LinkedIn ───────────────────────────────────────────────────────────
        result["linkedin"] = linkedin_url or self._extract_linkedin(contact_text)

        # ── GitHub ─────────────────────────────────────────────────────────────
        result["github"] = github_url or self._extract_github(contact_text)

        # ── Website ────────────────────────────────────────────────────────────
        result["website"] = portfolio_url or self._extract_website(
            contact_text,
    ...
    def _extract_github(self, text: str) -> Optional[str]:
        ...
        return select_canonical_github_profile(self._extract_github_urls(text))
    ...
    def _extract_github_urls(self, text: str) -> list[str]:
        ...
        return [match.group(0) for match in url_pattern.finditer(text or "")]
    ...
    def _extract_name(
        ...
        if hasattr(ner_engine, "extract_person_name"):
