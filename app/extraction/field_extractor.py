...
class FieldExtractor:
    ...
    def extract(
        ...
        Args:
            ...
            hyperlinks:   List of PDFHyperlink objects
        ...
        if hyperlinks:
            for link in hyperlinks:
                ...
                if "github.com" in uri:
                    github_urls.append(link.uri)
                elif "linkedin.com" in uri:
                    linkedin_url = link.uri
                elif "linkedin" not in uri and "github" not in uri:
                    # simplistic heuristic for portfolio: grab first remaining link (could be refined)
                    if not portfolio_url:
                        portfolio_url = link.uri

        github_urls.extend(contact_extractor._extract_github_urls(full_text))
        github_url = select_canonical_github_profile(github_urls)
