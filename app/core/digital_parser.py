...
class DigitalPDFParser:
    ...
    def _parse_with_pymupdf(self, file_path: Path) -> dict:
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                all_hyperlinks.extend(page_data.get("hyperlinks", []))
    ...
    def _extract_page_data(
        ...
        from app.models.schemas.pdf_hyperlink import PDFHyperlink
        for link in page.get_links():
            if link.get("kind") == fitz.LINK_URI:
                uri = link.get("uri", "")
                if uri:
                    hyperlinks.append(PDFHyperlink(
                        uri=uri,
                        page_number=page_num,
                        bounding_box=list(link.get("from", []))
                    ))
